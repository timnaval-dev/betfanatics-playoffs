...
const PLAYOFFS_DATA = {
  ...
  // Trader-set Stanley Cup probability overrides. Stored as { TEAMSHORT: probability }.
  // Any team not in this map uses the MoneyPuck-derived value. Conference winner probs
  // are derived from these via P(WinConf) = P(SCF) / P(WinSCF | WinConf).
  // Set in admin via the Outrights tab (American odds input → probability).
  outrightOverrides: {
    scf: {}
  },

  // Per-team conference probability biases (percentage point deltas).
  // Net delta is redistributed equally across unbiased teams in the same conference.
  // Only conference biases are used now — SCF is controlled directly via outrightOverrides.
  outrightBiases: {
    eastConf: {},
    westConf: {},
    scf: {}
